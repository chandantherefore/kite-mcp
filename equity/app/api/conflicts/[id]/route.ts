...
export async function POST(
  ...
  try {
    ...
    if (action === 'use_new') {
      ...
      await db.resolveConflict(conflictId, userId, 'resolved_manual', (session.user as any).email);
    } else if (action === 'ignore') {
      await db.resolveConflict(conflictId, userId, 'ignored', (session.user as any).email);
    }
...
export async function DELETE(
  ...
  try {
    ...
    await db.deleteConflict(conflictId, userId);
